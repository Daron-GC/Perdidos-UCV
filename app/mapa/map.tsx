...
export default function MapPage() {
  ...
  return (
    <div className="relative mx-auto max-w-md h-[900px] bg-[#eef5f3] overflow-hidden font-sans shadow-2xl rounded-[40px] border border-gray-200">
      ...
      {/* [Cuarto problema]: Se eliminó por completo el div 'absolute inset-0 z-10 pointer-events-none' */}

      {selectedUbicacion && (
        <div className="absolute bottom-36 inset-x-4 z-30 px-2">
          <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs text-cyan-500 font-semibold">Lugar seleccionado</p>
            ...
            <div className="mt-3 space-y-2">
              {selectedUbicacion.horarios && (
                <p className="text-sm text-gray-600">Horario: {selectedUbicacion.horarios}</p>
              )}
              {selectedUbicacion.descripcion && (
                <p className="text-sm text-gray-600">{selectedUbicacion.descripcion}</p>
              )}
            </div>
            ...
            <div className="mt-4 rounded-2xl bg-gray-50 p-3">
              ...
              <textarea
                rows={3}
                placeholder="Escribe un comentario sobre este lugar..."
                className="mt-2 w-full rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm outline-none focus:border-cyan-400"
              />
              <button className="mt-2 w-full rounded-xl bg-cyan-500 px-3 py-2 text-sm font-semibold text-white">
                Enviar comentario
              </button>
            </div>
          </div>
