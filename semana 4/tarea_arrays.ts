...
const nombres: string[] = productos.map(index => index.nombre)
console.log("2a. Nombres:", nombres)

// 2b. Crea un array de objetos con solo { id, nombre, precio }
//     (sin categoría, tags ni disponible)
const resumen: any[] = productos.map(index => {index.id,index.nombre,index.precio})
...
const preciosNuevos:number[] = productos.map(index => index.precio * 1.1)
...
const producto = productos.find(index => index.id=="p5")
console.log("3a. Producto p5:", producto?.nombre) // "Arepa reina"

// 3b. Encuentra el primer producto de la categoría "snacks"
const primerSnack = productos.find(index => index.categoria=="snacks")
...
const noExiste = productos.find(index => index.id=="p99")
if (noExiste) {
...
const tienePopular: boolean = producto?.tags.includes("popular") || false
...
const naturales: string[] = productos.filter(index => index.tags.includes("natural")).map(index => index.nombre)
...
const bebidasDisponibles: string[] = productos.filter(index => index.tags.includes("bebidas")).map(index => index.nombre)
...
const masCaro: Producto | undefined = productos.filter(index => index.nombre).reduce((max, actual) => actual.precio > max.precio ? actual : max)
...
//     Devuelve boolean
const haySnackPopular: boolean = productos.filter(index => {index.categoria=="snacks" && index.disponible && index.tags.includes("popular")}).some(index => index.tags.includes("popular"))